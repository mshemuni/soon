...
from ninja import Router, FileEx
...
from soon_aip.schemas import GPOSchema, ScriptsSchema
...
@router.get('/', response={200: List[GPOSchema], 500: str}, tags=["GPO"], description="Returns all GPOs")
def get_gpos(request):
    try:
        gpos = settings.gpo.get()
        return 200, [gpo_dataclass_to_schema(gpo) for gpo in gpos]
    except Exception as e:
        return 500, f"{e}"


@router.get('', response={200: GPOSchema, 400: str, 404: str, 500: str}, tags=["GPO"], description="Returns a GPO")
def get_gpo(request, uuid: str):
    try:
        gpo = settings.gpo.get(uuid)
        return 200, gpo_dataclass_to_schema(gpo)
...
@router.get('/scripts', response={200: ScriptsSchema, 400: str, 404: str, 500: str}, tags=["GPO"],
...
@router.post('/', response={201: GPOSchema, 402: str, 500: str}, tags=["GPO"], description="Creates a GPO")
def create_gpo(request, name: str, container: Optional[str] = None):
    try:
        if not request.auth.is_staff:
            return 401, "Must be Staff"

        gpo = settings.gpo.create(name, containers=container)
        return 201, gpo_dataclass_to_schema(gpo)
...
@router.delete('/', response={200: str, 400: str, 404: str, 500: str}, tags=["GPO"], description="Deletes a GPO")
...
@router.patch('/link', response={200: GPOSchema, 400: str, 404: str, 500: str}, tags=["GPO"],
...
@router.patch('/unlink', response={200: GPOSchema, 400: str, 404: str, 500: str}, tags=["GPO"],
...
@router.patch('/script', response={200: ScriptsSchema, 400: str, 404: str, 500: str}, tags=["GPO"],
...
@router.delete('/script', response={200: ScriptsSchema, 400: str, 404: str, 500: str}, tags=["GPO"],
